# Scenario: WRITE HIT - Write-Through + No-Write-Allocate (Random)
# Expected: Write updates cache + memory. Random eviction not triggered on hit.
...
# Step 3: Load block 3 → Miss → random eviction (0x8000 or 0x8001)
addi t0, t0, 512        # t0 = 0x10000400
lw t3, 0(t0)            # addr 0x10000400 → Tag 0x8002

# Step 4: Reload block 3 → should be a **HIT**
lw t4, 0(t0)            # addr 0x10000400 → Tag 0x8002

# Step 5: Write to block 3 → should be a **WRITE HIT**
sw t5, 0(t0)            # addr 0x10000400 → Tag 0x8002
